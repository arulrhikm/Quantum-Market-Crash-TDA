...
\langle \Pi_j \rangle$. The deflation penalty~\eqref{eq:deflation} introduces additional terms that are rational functions of the correlators (due to the normalization in the Rayleigh quotient), so the formal conditions of the proof are not exactly satisfied. In our reproduced implementation, gradient-variance fits over $n=4$--$12$ qubits ($j=0,1,2$) do not show vanishing gradients over the tested range; Section~\ref{sec:bp_results} reports the measured slopes and discusses this finite-size empirical behavior.
...
eigendecomposition times of approximately $41$~ms to $817$~ms per window
(mean $\approx 220$~ms) on a single CPU core (Colab runtime). The total
pipeline runtime for all 190 windows is approximately $63.8$~seconds.
...
In our current implementation, the measured log--log slopes are
approximately $+0.22$ ($j{=}0$), $+0.70$ ($j{=}1$), and $+0.56$ ($j{=}2$).
These values indicate that gradient variance does not vanish with system
size over the tested range; i.e., we do not observe barren-plateau-like
collapse in this experiment. This remains a finite-size empirical check,
...
\begin{figure}[t]
  ...
  \caption{Gradient variance vs.\ $n$ for $j = 0, 1, 2$ deflation
    ...
    slopes from the reproduced run (approximately $+0.22$, $+0.70$, $+0.56$)
    indicate non-vanishing gradients over the tested range.}
...
days; 100~of 190~windows satisfy this criterion in the reproduced run. The volatility proxy is
...
Using this procedure with the verified data yields precision $\approx 0.945$, recall $\approx 0.860$,
$F_1 \approx 0.901$, and ROC AUC $\approx 0.949$. The volatility-proxy baseline attains ROC AUC
$\approx 0.903$. In this reproduced run, the $\beta_1$ classifier outperforms the volatility proxy on ROC AUC.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{Figures/fig_roc_classification.png}
  \caption{ROC curve ($\beta_1$ AUC~$\approx 0.949$ vs.\ volatility proxy
    AUC~$\approx 0.903$) and precision/recall/$F_1$. Crash: ${>}10\%$ drawdown
    within 90~days; 100/190 positive windows.
    Values correspond to the verified dataset and the $F_1$-optimal in-sample thresholds.}
  \label{fig:roc_classification}
\end{figure}

These results suggest that the $\beta_1$ signal is informative for crash-risk
screening under the reproduced label definition, and can complement volatility-based
features in practical early-warning pipelines.

\subsection{Cost Crossover with Classical Methods}

The CE benchmark processed all 190~windows in approximately 63.8~s total (mean
220~ms/window eigendecomposition time). Edge counts $n_k$ varied from 31 to 429 (mean 136,
...
\begin{itemize}
    ...
    especially for large $\beta_k$. Our experiments show correct recovery up to $\beta_1 = 3$ but failure at $\beta_1 = 4$. For the financial application, the reproduced in-sample classifier uses an operating threshold near $\beta_1=2$ with precision $\approx 0.945$ and recall $\approx 0.860$, so reliable low-order null-space recovery remains important. For higher $\beta_k$, alternative strategies (SSVQE, weighted subspace search) would be needed.
...
under 820~ms per window in our reproduced run (about 63.8~s total for all 190 windows).
